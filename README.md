...
命令基本格式：
```bash
uv run bookmarkit [PDF文件路径] [--toc <目录页码范围>] [--first <正文第一页对应物理页码>]
```
...
## 详细参数参考
- `pdf_path` (位置参数)：待处理的原 PDF 文件路径。如果不提供，程序会在运行时提示输入。
- `--toc` (选项)：PDF的物理目录起始与结束页。请注意，这里的页码是人们习惯的 `1-based` (第一页就是1)。支持例如 `"1-3"` (代表1,2,3) 或者散装 `"1,2,3"`。如果不提供，程序会在运行时提示输入。
- `--first` (选项)：文档真正的第一页 (阿拉伯数字为1的页面) 在未处理的 PDF 文件中的**绝对物理页数** (也是 1-based)。这用于校准 VLM 抽取的书签相对跳转。如果不提供，程序会在运行时提示输入。
